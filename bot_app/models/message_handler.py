...
class MessageHandler:
    ...
    def get_task_from_json(self, json_obj: dict) -> list[PrevTask]:
        ...
        raw_tasks = map(Task.from_dict, json_obj['issues'])
        formed_tasks = map(self._create_prev_task, raw_tasks)

        return list(formed_tasks)

    def get_task_from_list(self, task_list: list[Task]) -> list[PrevTask]:
        """ Формирует список PrevTask из списка """
        formed_tasks = map(self._create_prev_task, task_list)
    ...
    def _create_prev_task(self, task: Task):
        return PrevTask(
            key=task.key,
            title=task.fields.summary,
            country=task.fields.customfield_10002.name,
            executor=self._get_executor(task),
            contact=task.fields.creator.displayName,
